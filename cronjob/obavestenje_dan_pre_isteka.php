		...
		if($datum_istMinusDanPre === $danas){
			...
			//poslednji predracun, da uhvatim generisan_url, tj. PDF za slanje 
				...
				foreach ($poslednji_predracun as $poslednji_predrac) {
					...
					if($uplata === $poslednji && $pdf_predracun != null && $poslednji_predrac->poslat_dan_pre !=1){
						$x =  '<b>'.$naziv.'</b> - истиче: <b>'.date('d.m.Y',strtotime($datum_ist)).'</b> '.$pdf_predracun.'<br>';
						//array_push($niz, $x);

						//$email = Predstavnik::getPredstavnik($id_suosnivac)->email;
						...
						if(!$mail->send()) {
						  ...
						  echo 'Email error:<pre> ' . $mail->ErrorInfo.'</pre>';
							...
							$poslat = Predracuni::updatePoslatDanPre($poslednji);
