...
$x.= '<tr><td style="border: 1px solid #ddd;padding: 8px;">РБ:</td><td style="border: 1px solid #ddd;padding: 8px;">Датум креирања рачуна:</td><td style="border: 1px solid #ddd;padding: 8px;">Број рачуна:</td><td style="border: 1px solid #ddd;padding: 8px;">Назив суоснивача:</td><td style="border: 1px solid #ddd;padding: 8px;">Линк ка предрачуну:</td></tr>';

$racuni = Racuni::getRacunePoslednjihMesecDana();
foreach ($racuni as $rac) {
	$datum_uplate = Uplate::getUplatuPoId($rac->id_uplate)->datum;
	$urlrac = explode('/',trim($rac->generisan_url));
    $url = $urlrac[1];
    
	$naziv_suos = Suosnivac::getSuosnivac($rac->id_suosnivac)->naziv;

	$x.= '<tr><td style="border: 1px solid #ddd;padding: 8px;">'.$redni_broj.'. </td><td style="border: 1px solid #ddd;padding: 8px;">'.date('d.m.Y.',strtotime($datum_uplate)).'</td><td style="border: 1px solid #ddd;padding: 8px;">'.$rac->broj_racuna.'</td><td style="border: 1px solid #ddd;padding: 8px;">'.$naziv_suos.'</td><td style="border: 1px solid #ddd;padding: 8px;"><a href="10.10.10.233/inc/racuni/'.$url.'" target="_blank">'.$url.'</a></td></tr>';
	$redni_broj++;
}
$x.= '</table>';
...
$mail->CharSet = 'UTF-8';
$mail->Subject = 'Издати рачуни';
$mail->Body = '<h2 style="color:red;">Издати рачуни:</h2><p>'.$x.'</p>';
