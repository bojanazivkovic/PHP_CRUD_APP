...
//izvlacim isticanje za poslednju uplatu - isto koristim za cron za neaktivne
	...
	$datum_istPlusJedanDan = strtotime(date($datum_ist, strtotime($datum_ist)). '+1 days');
	$datum_istPlusJedanDan = date('Y-m-d', $datum_istPlusJedanDan);

	//$datum_istMinusDanPre = strtotime(date($datum_ist, strtotime($datum_ist)). '-1 days');
	//$datum_istMinusDanPre = date('Y-m-d', $datum_istMinusDanPre);

	
		if($datum_istPlusJedanDan === $danas){
			...
			//poslednji predracun, da uhvatim generisan_url, tj. PDF za slanje 
				...
				foreach ($poslednji_predracun as $poslednji_predrac) {
					...
					if($uplata === $poslednji && $pdf_predracun != null && $poslednji_predrac->poslat_dan_posle !=1){
						...
						$mail->Body = '<div style="font-family:Roboto Light">Поштовани/а, <br><br>Подсећамо Вас да је Ваша Годишња накнада истекла '.$datum_isticanja.'. Након овог датума имате могућност да у наредних месец дана обновите годишњу накнаду како не бисте изгубили статус суоснивача.<br><br>Уколико сте већ извршили уплату, молимо Вас да занемарите ову поруку. Систем је генерисао поруку јер уплата није евидентирана. Ваша уплата биће евидентирана чим буде прокњижена на наш рачун.<br><br><br>Поздрав,<br><br><b>Канцеларија РНИДС</b><br><br><table style="font-family:Roboto Light"><tr><td><img src="cid:logoimg" /></td><td><span style="font-size:12px">Фондација "Регистар националног интернет домена Србије"</span><br><span style="font-size:10px">Жоржа Клемансоа 18а/I, 11108 Београд, ПАК 101147<br>Tel: 011.7281.281  |  rnids.rs   |  домен.срб<br>domen.rs   |   домен.срб   |   FB   |   TW   |   LN   |   YT</span></td></tr></table></div>';
						...
						if(!$mail->send()) {
						  ...
						  echo 'Email error:<pre> ' . $mail->ErrorInfo.'</pre>';
							...
							$poslat = Predracuni::updatePoslatDanNakonIsteka($poslednji);
